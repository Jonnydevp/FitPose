...
class VideoProcessor:
    ...
    def analyze_movement_patterns(self, frames_data: List[Dict], expected_exercise: Optional[str] = None) -> Dict:
        ...
        wrist_y_range = max(wrist_y_s) - min(wrist_y_s)  # normalized [0..1]
        ...
        if expected_exercise:
            ...
            # Squat: large knee flexion and noticeable body vertical displacement
                ...
                exercise_type = "squat"
            # Deadlift: moderate knee flexion and hip hinge, less vertical displacement than squat
        ...
        # Exercise-specific rep counting via state machines with hysteresis
        def count_reps_pullup(y_series: List[float]) -> int:
            y_min, y_max = min(y_series), max(y_series)
            amp = y_max - y_min
            if amp < 0.03:
                return 0
            low = y_min + 0.25 * amp   # "up" zone (lower y)
            high = y_min + 0.75 * amp  # "down" zone
            state = 'down'
            reps = 0
            hold = 0
            for y in y_series:
        ...
        if exercise_type == 'pullup':
            ...
            # Fallback: rough estimate from combined ranges
